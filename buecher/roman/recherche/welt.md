# Welt / Setting

Alle Angaben sind aus `recherche/quellenmaterial/` belegt (Vorrangregel:
`kap3_korrigiert_20260707.md` > `gesamtdokument_20261224.md` >
`band1_dobson_neun.md`). Empfehlungen/Analysen aus TEIL A des
Gesamtdokuments sind ausdrücklich als **Vorschlag, nicht Kanon**
markiert. Als Ordnungs-/Referenzprinzip verwendet dieses Dokument
durchgehend **Handlungsstränge**, nicht die im Rohmaterial vergebenen
Kapitelnummern — siehe `recherche/zeitleiste.md` für die Begründung.

## Reihenaufbau: zwei Bücher + Rahmen + historische Ebene

```
RAHMEN (Gegenwart, 2018)
  Corinna Brandenburg erhält Joosts Nachlass (Hamburg-Ottensen,
  16. Jan. 2018) → E-Mail-Kontakt zu einem anonymen Informanten
  (ab Feb. 2018) → Reise nach Dubai, Treffen mit "Daily" McAllan
  (10.–12. Okt. 2018, siehe kap3_korrigiert)
        ↕
RÜCKBLICK / BUCH 1 — "Das Geheimnis der »Dobson Neun«" (2011–2015)
        ↕
BUCH 2 — "Dobneun Son — Die Helfenden Hände"
  (schließt direkt an Buch 1 an: >>> London, Juni 2015 <<<)
        ↕
HISTORISCHE EBENE — Königreich Baekje, Korea, 650–660 AD
  (wird Corinna als Akte/Recherchenotiz Joosts vermittelt)
```

Diese Klammerstruktur (Rahmen–Rückblick–historische Ebene) ist in TEIL A
als Gestaltungsempfehlung des Ghostwriter-Gutachtens beschrieben, aber
durch `kap3_korrigiert` (Corinna liest/erlebt die Rückblende real) und
die tatsächlichen Manuskript-Abschnitte (Buch 1, Dobneun-Son/Helfende-
Hände-Strang/Zentrale etc.) inzwischen weitgehend im Manuskript
umgesetzt.

## Zeitebenen im Überblick

1. **650–660 AD** — Königreich Baekje (Korea): drei Kaufleute (Hyeop,
   Mok, Baek), Ursprung des Konflikts um verstecktes Vermögen. Im
   Manuskript nur als Einstiegsszene ausgeschrieben (Einladung zum
   gemeinsamen Essen); die von TEIL A skizzierte Fortsetzung
   (Seeschlacht, Flucht nach Taiwan, Gründung der Helfenden Hände) ist
   **Konzept, nicht geschriebener Text**.
2. **2011–2015** — Haupthandlung Buch 1 (New York, Pyeongyang, Moskau,
   Brisbane/Australien, Kuala Lumpur/Südchinesisches Meer, Assen/Den
   Haag/Groningen, Hamburg, Donbass/Amsterdam, Malaysia, Libyen/Westsahara,
   London).
3. **Juni 2015 ff.** — Buch 2 (London-Anschlag, danach geplante
   Abschnitte zu Korea, "Feinde"/Gutseoda, Afrika/UnMaCo, Recherche/Geld
   — Stand Manuskript: nur die London-Szene ist vollständig
   ausgeschrieben, Rest sind Platzhalter, siehe Zeitleiste).
4. **2018** — Rahmenhandlung Corinna/McAllan (Hamburg, Dubai, Wüste).

## Wichtige Schauplätze

- **New York** — UN-Gebäude, Imbiss "Moon's JAM" (Tarnung/Treffpunkt
  nordkoreanischer und chinesischer Diplomaten), Ort von Klaus Richters
  Tod (2011).
- **Pyeongyang / Nordkorea** — Büro des "obersten Führers", "Office 121",
  Bunker Minister Kim Wong-Hong, Marinewerft Sinpo (U-Boot-Umbau),
  Tonghae-Testgelände (BUK-Test).
- **"Weinender Berg" (Korea, nahe Juwangsan-Nationalpark)** — verstecktes
  Haupt-HQ der Helfenden Hände/Dobneun-Son, in einer Grotte hinter einem
  Wasserfall, historisch aus einem Mönchskloster/Heilzentrum
  hervorgegangen ("Chiyu Haneun Gohyang"/"Chi-yu-i jeol"). **Kanon** laut
  dem Dobneun-Son/Helfende-Hände-Strang in Buch 1 — nicht mehr offene
  Frage (siehe Zeitleiste/Konsistenz).
- **Taiwan** — zweites HQ der Helfenden Hände (Finanzen/Wirtschaft);
  Ostküste als vermuteter Ort der 650-AD-Schätze (nur als
  Analyse-Hypothese in TEIL A, nicht im Manuskripttext bestätigt).
- **Moskau / Russland** — Vorbereitung der BUK-Rakete (Hauptverwaltung
  für Raketen und Artillerie, Kursk, 53. Luftabwehrbrigade), Verbindung
  zum MH17-Abschuss.
- **Brisbane / Glashouse Mountains, Australien** — paralleler
  Vorbereitungsstrang, Jan. 2014.
- **Kuala Lumpur / Sepang Race Circuit / Cargo-City** — Ausgangspunkt und
  Operationszentrum der MH370-Entführung (März 2014); auch Sitz der
  malaysischen Special Branch (Bibi Kaur, Satria Chow).
- **Südchinesisches Meer / Südpazifik (Hohotobei, Tobi Island, Helen
  Reef)** — Landung/Bergung von MH370 (real angelehntes Ereignis, hier
  fiktional als U-Boot-gestützte Operation ausgestaltet).
- **Assen, Den Haag, Groningen, Zwolle (Niederlande)** — AIVD-Strang um
  Anique Smits, Thomas Ganghofer, Joost van der Dijkstraat, Piet Klocke,
  Flim Dijkwijk; führt zu MH17 (Juli 2014).
- **Hamburg (Ottensen, Deichstraße 24)** — Corinna Brandenburgs
  Wohnort/Redaktionsumfeld; Joosts Wohnung mit den zentralen Akten.
- **Donbass / Snizhne, Amsterdam, Den Haag** — MH17-Abschuss (17. Juli
  2014) und Ermittlungsstränge danach.
- **Malaysia, Royal Belum National Park** — Special-Branch-Einsatz
  (Feb. 2015), Bibi Kaur/Satria Chow.
- **Libyen / Westsahara** — Expedition von Dr. Benjamin Dingaka
  (Bodenproben, vermutetes Goldvorkommen), bewacht von Bibi Kaur und
  Satria Chow.
- **London** — Wohnsitz/Labor Dr. Dingakas (Luftschutzbunker als
  Probenlager); Schauplatz des vereitelten nordkoreanischen Anschlags
  (Buch 2, Juni 2015): Hyde Park (Drohnenüberwachung), DSCT/Scotland
  Yard, SAS Regent Park.
- **Dubai (The Palm, One&Only-Resort, Wüste/Al Maha Resort)** — 2018,
  Rahmenhandlung Corinna/McAllan; Ort des "Nachtfalken"-Anschlags.

## Reale Bezüge (Weltenbau-Grundlage, aus dem Material)

- **MH370 (Malaysia Airlines Flug 370, 2014):** Grundlage für die
  fiktionale Entführungs-/Bergungsoperation im Südpazifik/Nordkorea.
- **MH17 (Malaysia Airlines Flug 17, 2014):** Grundlage für den
  BUK-Abschuss über dem Donbass; im Manuskript mit präzisen
  Datums-/Uhrzeitangaben (Donbass, 15./16./17. Juli 2014) nachgebaut,
  inkl. bekannter Logikfehler (siehe Zeitleiste: BUK-Ankunft vs.
  Abschusstag).
- **Korea 650–660 AD, Königreich Baekje:** Historisch belegte
  Rahmendaten (Untergang 660 AD durch Silla/Tang-Allianz, Hauptstadt
  Sabi/Busosanseong, Herrscher Mu und Uija) werden im Manuskript korrekt
  zitiert (inkl. Handschriftzitat "Prof. Mark Peterson, Brigham Young
  Universität" als fiktive Rechercheanmerkung Joosts).
- **Falklandkrieg 1982:** Wird als historischer Beleg für die Reichweite
  der Helfenden Hände angeführt (Vorwarnung an Frau Thatcher,
  Frachtschiffe verlassen Portsmouth am 02.03.1982).
- **Taiwan-Konflikt/Geopolitik, TSMC/Halbleiterproduktion:** In TEIL A
  als **Analyse/Begründungsversuch** ausgearbeitet (warum ein
  chinesischer Angriff auf Taiwan strategisch unattraktiv wäre) — das ist
  Hintergrundrecherche/Argumentation des Ghostwriter-Gutachtens, **nicht
  zwingend als Dialog-/Erzähltext im Manuskript übernommen**, sondern als
  Steilvorlage für die Motivforschung "Warum Taiwan wirklich?".
- **Sun Tzu, "Die Kunst des Krieges":** Im Rohmaterial ist praktisch
  jeder Buch-1-Abschnitt (im Quelltext ursprünglich nummeriert als
  "Kapitel 2" bis "Kapitel 14") mit einem Sun-Tzu-Zitat als Motto
  versehen. Diese Nummerierung wird für dieses Projekt nicht als
  Ordnungsprinzip übernommen (siehe `zeitleiste.md`, Abschnitt
  "Handlungsstränge in Buch 1" für die stattdessen verwendete
  themenbasierte Gliederung).

## Organisationsregeln der Welt (Kanon)

- Die **Helfenden Hände / Dobneun-Son** verwenden zur Tarnung interne
  Decknamen nach Körperteilen ("Mutter", "rechter/linker Arm",
  "rechtes/linkes Auge", "rechte/linke Schulter" am Taiwan-HQ), damit
  nie echte Namen fallen. Führung stets weiblich ("Mutter"/Eomeoni).
  Sitz: "weinender Berg" (Korea) + zweites HQ Taiwan (Wirtschaft/Finanzen).
- Operative Feldzellen/Teams (z.B. in Südkorea, Malaysia, Niederlande,
  London) scheinen zusätzlich eigene Decknamen zu verwenden (z.B.
  "Oenjjog Jumeog"/Linke Faust in Südkorea) — ob das derselbe
  Codesystem wie die HQ-Führung ist oder ein eigenes Schema für
  Feld-Operative, ist nicht abschließend geklärt (siehe Figuren.md,
  Offene Fragen).
- Gegenorganisation: **Gutseoda**, HQ **"Falkennest"** (Nordkorea, nahe
  Ch'oso-dong), nur dem "obersten Führer" unterstellt, jahrhundertealte
  Feindschaft mit den Helfenden Händen. Verhältnis zu den 2018 in Dubai
  auftretenden **"Nachtfalken"** ungeklärt (siehe Offene Fragen in
  `figuren.md`).
- Die Finanzierung der Helfenden Hände wird im Dobneun-Son/Helfende-
  Hände-Strang (Zentrale) explizit benannt: eigenes
  "Finanzierungsprogramm", Kontakte/Kandidaten bei Zentralbanken
  (genannt: Deutsche Bundesbank, alternativ Singapur) — passend zur
  TEIL-A-Hypothese, dass Taiwan/die 650-AD-Schätze die wirtschaftliche
  Basis der Organisation bilden (dort aber ausdrücklich als
  offene/unbestätigte Idee markiert).

## Offene Strukturfragen aus TEIL A (Analyse, nicht Kanon)

Diese Punkte sind Diskussionsstand des Ghostwriter-Gutachtens vom 23./24.
Mai 2026 und sollten dem Nutzer zur Entscheidung vorgelegt werden, bevor
sie als Kanon behandelt werden:

- Soll McAllan konsequent als Ich-Erzähler mit auktorialem Rückblick
  auftreten (Empfehlung), oder bleibt die im Manuskript vorherrschende
  wechselnde auktoriale/personale Perspektive bestehen?
- Wie viele "Treffen" umfasst die Rahmenhandlung (zwei? drei? siehe
  Rechercheaufträge-Liste in der Zeitleiste)?
- Was genau sind die "Schätze" der Kaufleute von 650 AD (Gold, Schwefel,
  Dokumente/Wissen, Kombination) — laut TEIL A bewusst offengehalten
  "bis zur Redaktion".
- Ist Taiwan wirklich der wirtschaftliche Unterbau der Helfenden Hände
  über 1.400 Jahre (starke TEIL-A-Hypothese) — im Manuskripttext bisher
  nicht bestätigt, aber die Existenz eines Taiwan-HQ
  (Dobneun-Son/Helfende-Hände-Strang, Buch 1) stützt die Idee.
